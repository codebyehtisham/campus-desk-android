#!/usr/bin/env zsh
# Build, install, and launch Campus Desk RELEASE on a connected Android device.
# Release: production API, DEV_TOOLS=false (no shake logs, no env picker).
set -euo pipefail
...
echo "==> Building and installing RELEASE APK (production, no shake / env picker)..."
{
  echo "=== $(date) ==="
  echo "device=$DEVICE"
  echo "variant=release"
  ./gradlew :app:installRelease --no-daemon
} 2>&1 | tee "$LOG"

echo "==> Launching $APP_ID (release)..."
adb shell am force-stop "$APP_ID" >/dev/null 2>&1 || true
adb shell am start -n "$APP_ID/$MAIN_ACTIVITY" \
  -a android.intent.action.MAIN \
  -c android.intent.category.LAUNCHER

echo "Done. Release app launched on $DEVICE."
echo "API: production | DEV_TOOLS: off"
echo "Log: $LOG"
